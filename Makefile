#
# Toplevel Makefile for JIP2Location.
#
...
DIRS = src/com/ip2location
ifdef CYGPATH
JAR_FILE = "$(shell cygpath -C UTF8 -w "$(LIB_DIR)/jip2location-$(MAJOR).$(MINOR).$(MICRO).jar")"
CLEAN_LIST = $(LIB_DIR)/jip2location-$(MAJOR).$(MINOR).$(MICRO).jar
JAVADOC_DEST = "$(shell cygpath -C UTF8 -s -w "$(DEST)$(JAVADOC_DIR)")"
JAVADOC_SRC = "$(shell cygpath -C UTF8 -s -w "$(JAVA_SRC_DIR)")"
else
JAR_FILE = $(LIB_DIR)/jip2location-$(MAJOR).$(MINOR).$(MICRO).jar
CLEAN_LIST = $(JAR_FILE)
...
all: dir default-target
	cd classes; $(JAR) cvf $(JAR_FILE) com
